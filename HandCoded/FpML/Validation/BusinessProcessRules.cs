...
namespace HandCoded.FpML.Validation
{
	...
	public sealed class BusinessProcessRules : FpMLRuleSet
	{
        ...
        public static readonly Rule	RULE01
		    = new DelegatedRule (Preconditions.R4_1__LATER, "bp-1", new RuleDelegate (Rule01));
		...
		private static bool Rule01 (string name, NodeIndex nodeIndex, ValidationErrorHandler errorHandler)
		{
            if (nodeIndex.HasTypeInformation)
                return (Rule01 (name, nodeIndex, nodeIndex.GetElementsByType (DetermineNamespace (nodeIndex), "FirstPeriodStartDate"), errorHandler));
			return (Rule01 (name, nodeIndex, nodeIndex.GetElementsByName ("firstPeriodStartDate"), errorHandler));
		}
		
		private static bool Rule01 (string name, NodeIndex nodeIndex, XmlNodeList list, ValidationErrorHandler errorHandler)
		{
			bool		result 	= true;
		...
		{
            if (nodeIndex.HasTypeInformation)
                return (Rule02 (name, nodeIndex, nodeIndex.GetElementsByType (DetermineNamespace (nodeIndex), "FirstPeriodStartDate"), errorHandler));
			return (Rule02 (name, nodeIndex, nodeIndex.GetElementsByName ("firstPeriodStartDate"), errorHandler));
		}
