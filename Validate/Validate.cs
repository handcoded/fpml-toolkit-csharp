...
namespace Validate
{
	...
	sealed class Validate : Application
	{
        ...
        /// <summary>
        /// The <see cref="Option"/> instance used to detect <b>-report</b>
        /// </summary>
	    private Option			reportOption
		    = new Option ("-report", "Generate an XML report of the results");
    	
        /// <summary>
        /// The <see cref="Option"/> instance used to detect <b>-output</b>
        /// </summary>
		...
		{
            if (reportOption.Present)
                writer.WriteLine ("\t\t<syntaxError>" + args.Message + "</syntaxError>");
            else
		...
		{
			if (reportOption.Present)
                writer.WriteLine ("\t\t<validationError rule=\"" + rule
						+ "\" context=\"" + XPath.ForNode (context)
						+ "\"" + ((data != null) ? (" additionalData=\"" + data + "\"") : "")
						+ ">" + description + "</validationError>");
			else {
			    if (data != null)
				    writer.WriteLine (rule + " " + XPath.ForNode (context) + " " + description + " [" + data + "]");
			    else
				    writer.WriteLine (rule + " " + XPath.ForNode (context) + " " + description);
            }
		}
	}
}
